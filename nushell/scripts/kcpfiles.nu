def main [--deployment (-d): string, --volume (-v): string, --path (-p): path, --namespace (-n): string, --wait (-w): duration = 2sec, --wait-times: int = 10] {
  print $"Copying from the volume ($volume) from the deployment ($deployment) to ($path)"
...
spec:
  ...
  print $"Moving data from the container to the path: ($path)"
  try {
    kubectl cp $"nuguish-volupload-($deployment):/data" $path
  } catch {|err|
